...
for (footnote of footnotes) {
  footnote.addEventListener("mouseover", (e) => {
    let anchor = document.querySelector(
      `[data-anchor='${e.currentTarget.dataset.footnote}']`
    );
...
for (anchor of anchors) {
  anchor.addEventListener("mouseover", (e) => {
    let footnote = document.querySelector(
      `[data-footnote='${e.currentTarget.dataset.anchor}']`
    );
...
window.addEventListener("mouseup", function () {
  if (!sel.isCollapsed) {
    ...
    tooltip.style.top =
      ((r.bottom - rb2.top) * 100) / (rb1.top - rb2.top) + "px"; //this will place ele below the selection
    tooltip.style.left =
...
function save() {
  ...
  infoNotes.innerHTML =
    "Notes sauvegardées le " +
    jour +
    " à " +
    heure +
    "<br><br>pablomoreno@proton.me";
  ...
  data =
    ...
    "<header><p>The Web We Want</p><p>L’importance d’imaginer et de concevoir des espaces web<br>fait-main pour enrichir nos vies numériques</p><p>Pablo MORENO → Mémoire DNSEP → ESAD Amiens 2023/2024</p></header>" +
    body.innerHTML +
    "</html>";
